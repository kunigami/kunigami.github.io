...
We'll start with blocking I/O, the explore a few different ways to perform non-blocking I/O such as busy loops, `select()`, `epoll()` and finally cover the library `libevent`.
...
## Non-Blocking I/O, Kernel Space: select
...
## Non-Blocking I/O, Kernel Space: epoll
