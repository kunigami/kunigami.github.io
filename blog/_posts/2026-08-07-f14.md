...
The other is power of 2. Primes are slower to apply `%` of (we cannot use bit shifting as with powers of 2), but they tend to hash more uniformly. In any case, every time the number of buckets changes, we need to rehash.
...
In [Velox: The Vector]({{blog}}/2026/06/01/velox-vectors.html) we discussed the dictionary-encoding in which we only store indices to the actual underlying data, similar to `F14VectorMap`. In [Velox: Memory](https://www.kuniga.me/blog/2026/08/04/velox-memory.html) we mentioned that memory is never given back to the OS, which is the same behavior for `F14VectorMap`, it never shrinks the number of buckets. It makes sense statistically to not shrink given that it's expensive and that, without more information, we can assume we'll need the peak memory in the future.
