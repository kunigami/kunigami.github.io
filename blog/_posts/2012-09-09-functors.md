...
A map receives a function that transforms an element of type `a` into an element of type `b` and a list of elements of type `a`. We can see this by checking the its type on ghci:
