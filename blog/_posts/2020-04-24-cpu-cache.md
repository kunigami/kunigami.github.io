...
Like a hash table with a simple function. Each set has exactly one cache line. If a new entry maps to a row already "occupied", the previous entry is evicted.
...
In this case, each set has 2 cache lines, so a given memory address can go to any of the two. When deciding which cache line to assign it to, we can use LRU (least recently used).
...
**m-way associative cache**

A more general structure would be `m` cache lines per set. This further shifts the balance between complexity (the higher the `m` the more expensive it is) and avoiding degenerate cases (resilient to up N hot addresses).
