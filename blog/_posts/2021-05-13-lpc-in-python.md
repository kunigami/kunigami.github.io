...
Let $x(t)$ be the amplitude of our signal at a given instant $t$. According to the source-filter model, it's generated by a source signal $e(t)$ going through a resonant filter $h(t)$.

$$x(t) = h(t) \cdot e(t)$$

The model further assumes that the current signal also depends on the past $p$ samples, that is $x_{t-1}, \cdots, x_{t-p}$, and that the source is constant, so effectively:
