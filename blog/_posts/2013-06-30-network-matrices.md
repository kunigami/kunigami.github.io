...
If $$M$$ has, for each column, at most one entry 1 and at most one entry -1, we can show it's a network matrix by constructing a digraph $$G$$ and a tree $$T$$ such that their corresponding network matrix is $$M$$. We first build a direct star $$T$$ with m vertices and with all edges pointing towards the center vertex $$v^*$$. We now build the digraph $$G$$ with the same vertex set. For each column in $$M$$ that has an entry +1 and -1 for rows $$(u,v^*)$$ and $$(w,v^*)$$ we add an edge $$(u,w)$$. For columns having a single entry for row $$(u, v^*)$$, we add the $$(u,v^*)$$ to $$G$$ if it's +1 or $$(v^*,u)$$ if it's -1.
