...
C++11 introduced smart pointers (`std::unique_ptr` and `std::shared_ptr`) which do have visibility on the lifetime of the object so they can delete it - note this is opaque to the C++ runtime. There are exceptions in the presence of cyclic references, which we discuss later in *Finalizers*.
...
} // releases memory. Calls ~MyClass 11 times
{% endhighlight %}
...
**Locks.** A final example is making sure locks are released, avoiding potential deadlocks. Example using `std::unique_lock`:
